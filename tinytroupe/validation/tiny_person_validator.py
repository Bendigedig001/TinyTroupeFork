...
class TinyPersonValidator:
    ...
    def validate_person(person, expectations=None, include_agent_spec=True, max_content_length=default_max_content_display_length) -> tuple[float, str]:
        ...
        cache_params = utils.prompt_cache_params_for_family("validation:check_person")
    ...
    @staticmethod
    async def validate_person_async(
        person,
        expectations=None,
        ...
        with open(
            check_person_prompt_template_path, "r", encoding="utf-8", errors="replace"
        ) as f:
            check_agent_prompt_template = f.read()
        ...
        cache_params = utils.prompt_cache_params_for_family("validation:check_person")
        message = await client().send_message_async(
            current_messages,
            response_format=ValidationResponse,
            enable_pydantic_model_return=True,
            **cache_params,
        ...
        while (
            ...
            if message.questions:
                ...
                questions_text += "\n".join(
                    [f"{i+1}. {q}" for i, q in enumerate(message.questions)]
                )
                ...
                await person.listen_and_act_async(
                    questions_text, max_content_length=max_content_length
                )
                responses = person.pop_actions_and_get_contents_for("TALK", False)
                logger.info(f"Person reply:\n{responses}")
                ...
                logger.warning(
                    "LLM did not provide questions but validation is not complete"
                )
                break

        if message is not None and message.is_complete and message.score is not None:
            logger.info(
                f"Validation score: {message.score:.2f}; Justification: {message.justification}"
            )
            return message.score, message.justification
        else:
            logger.error("Validation process failed to complete properly")
